...
def main(args):
    ...
    adv_samples = np.load("./zoo_adv_200.npy")
    ...
    distortion = np.concatenate([distortion])
    zoo_s = (distortion<=1)&(np.argmax(predictions_adv, axis=1) != labels)&(np.argmax(predictions_test, axis=1) == labels)
    print(np.sum(zoo_s))
    if args.method =='CW':
    ...
    np.save('./features/'+args.dataset+'/'+args.method+'_distortion',distortion)
    np.save('./features/'+args.dataset+'/'+args.method+'_pred_test',predictions_test)
    np.save('./features/'+args.dataset+'/'+args.method+'_pred_adv',predictions_adv)
