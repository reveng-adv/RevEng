...
def all_true():
    ...
    zoo_s = (zoo_l2 <=1)&(np.argmax(zoo_p_adv, axis=1) != labels) & (np.argmax(p_test,axis=1) == labels)
...
def split():
    ...
    '''
    methods = ['CW', 'ZOO','BOUNDARY','PGD', 'SQUARE','HSJ']
    comb = []
    pertb =[]
    attk_method = []
    label = []
    ...
    for method in methods:
        ...
        attk_method.append(np.repeat(j, comb_data.shape[0]))
        label.append(lab)
        i += 1
        j = i%3
