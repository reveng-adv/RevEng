...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb 28 21:44:43 2022
...
def combine(method = "CW"):
    ...
    #print(aa.shape)
    np.save(path+"nes_adv10k.npy",aa)
    ...
    aa = np.concatenate((a1,a2,a3,a4))
    print(aa.shape)
    np.save(path+"nes_distortion10k.npy",aa)
    ...
    aa = np.concatenate((a1,a2,a3,a4))
    print(aa.shape)
    np.save(path+"nes_pred_adv10k.npy",aa)
    ...
    aa = np.concatenate((a1,a2,a3,a4))
    print(aa.shape)
    np.save(path+"nes_pred_test10k.npy",aa)
    ...
    aa = np.concatenate((a1,a2,a3,a4))
    print(aa.shape)
    np.save(path+"nes_success10k.npy",aa)
    ...
    aa = np.concatenate((a1,a2,a3,a4))
    print(aa.shape)
    np.save(path+"nes_labels10k.npy",aa)
...
def triple_true():
    ...
    cwrs_s = (cwrs_l2 <=5) &(np.argmax(cwrs_p_adv, axis=1) != labels) & (np.argmax(p_test,axis=1) == labels)

    #bdr_s = (np.argmax(bdr_p_adv, axis=1) != labels) & (np.argmax(p_test,axis=1) == labels)
    
    ind = cw_s & cwnes_s & cwrs_s
    #print(np.where(cwnes_l2<1))
    #print(np.where((cw_l2[ind]<1) & (cwnes_l2<1) & (cwrs_l2<1)))
    print(np.sum(ind))
    np.save(path+"ind_triple_true",ind)
    print(np.max(np.load(path+"CW_distortion10k.npy")[ind]))
    print(np.max(np.load(path+"CWnes_distortion10k.npy")[ind]))
    print(np.max(np.load(path+"CWrs_distortion10k.npy")[ind]))
    print(np.mean(np.load(path+"CW_distortion10k.npy")[ind]))
    print(np.mean(np.load(path+"CWnes_distortion10k.npy")[ind]))
    print(np.mean(np.load(path+"CWrs_distortion10k.npy")[ind])) 
    print(np.median(np.load(path+"CW_distortion10k.npy")[ind]))
    print(np.median(np.load(path+"CWnes_distortion10k.npy")[ind]))
    print(np.median(np.load(path+"CWrs_distortion10k.npy")[ind]))

def plot():
    path = "./features/cifar10/"
    #path = "./"
    ind = np.load(path+"ind_triple_true.npy")
    cw = np.load(path+"CW_distortion10k.npy")[ind]
    cwnes = np.load(path+"CWnes_distortion10k.npy")[ind]
    #bdr = np.load(path+"BOUNDARY_distortion10k.npy")[ind]
    cwrs = np.load(path + "CWrs_distortion10k.npy")[ind]
    #cw = np.load(path+"CW_distortion10k.npy")
    #sq = np.load(path+"SQUARE_distortion10k.npy")
    #bdr = np.load(path+"BOUNDARY_distortion10k.npy")
    #zoo = np.load(path + "ZOO_distortion10k.npy")
    ...
    plt.savefig(path+"/distortionl2_visual")
...
def split(): 
    ...
    '''
    methods = ['CW','CWnes','CWrs'] #'SQUARE','BOUNDARY']
    comb = []
    pertb =[]
    attk_method = []
    label = []
    ...
    for method in methods:
        ...
        #distortion_data = np.load(path+method+"_distortion10k.npy")
    ...
    np.save("./features/combined/x.npy",x)
    ...
    #np.save("./features/cifar10/test_x.npy",x_test)
    np.save("./features/cifar10/train_adv.npy",adv_train)
    ...
    np.save("./features/combined/comb_xtrain.npy",x_train)
    np.save("./features/combined/comb_xtest.npy",x_test)
