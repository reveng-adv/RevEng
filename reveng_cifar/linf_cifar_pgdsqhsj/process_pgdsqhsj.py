...
def plot():
    ...
    pgd = np.load(path+"PGD_distortion10k.npy")[ind]
    sq = np.load(path+"SQUARE_distortion10k.npy")[ind]
    hsj = np.load(path+"HSJ_distortion10k.npy")[ind]
    ...
    plt.hist(pgd, 
         ...
         color='green',
         #bins = 30,
         range = (0.028,0.032)) # customized color parameter
    plt.hist(sq, 
            alpha=0.5,
           label='sq',
           #bins = 30,
...
def split(): 
    ...
    np.save("./features/cifar10/test_label.npy",label_test)
