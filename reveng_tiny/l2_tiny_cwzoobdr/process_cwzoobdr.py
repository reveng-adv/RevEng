...
def triple_true():
    ...
    #print(np.shape(sq_l2))
    zs = np.load(path + "ZOO_success10k.npy")
    zoo_s = (zoo_l2<5) & (np.argmax(zoo_p_adv,axis =1) != labels) & zs
    #zoo_s =(zoo_l2<=5.001)& (np.argmax(zoo_p_adv, axis=1) != labels) & (np.argmax(p_test,axis=1) == labels)
    print("zoo_s",np.sum(zoo_s))
    print("zoo_p_adv",np.sum((np.argmax(zoo_p_adv,axis =1) != labels)))
    #print("zoo_p_test",np.sum(np.load(path+"ZOO_pred_test10k.npy")))
    print(np.sum(np.argmax(p_test,axis =1) == labels))
...
def split(): 
    ...
    np.save("./features/tiny/test_adv.npy",label_test)
