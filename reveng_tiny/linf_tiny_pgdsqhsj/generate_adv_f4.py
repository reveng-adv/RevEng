...
from art.attacks.evasion.projected_gradient_descent.projected_gradient_descent_pytorch import ProjectedGradientDescentPyTorch as PGDAttack
#from carlart_xw/art/attacks/evasionini import CarliniL2Method
from art.estimators.classification import PyTorchClassifier
...
def main(args):
    ...
    classifier = PyTorchClassifier(
        model=model,
        clip_values=(0.0, 1.0),
        #clip_values =(-2.0860121,  2.6635718),
        loss=criterion,
        optimizer=optimizer,
    ...
    if args.method == 'CW':
        attack = CarliniLInfMethod(classifier=classifier,max_iter=args.cw_steps)
    if args.method == 'SQUARE':
        attack = SquareAttack(estimator=classifier, eps=0.03, max_iter=args.square_query,norm=np.inf)
    if args.method =='PGD':
        attack = PGDAttack(estimator = classifier, norm=np.inf,eps = args.pgd_eps,eps_step= 0.003,max_iter=20)
    if args.method == 'HSJ':
        attack = HopSkipJump(classifier = classifier, norm = np.inf,max_iter =args.hsj_iter,max_eval = 1000)
    '''
    if args.method == 'CW':
        attack = CarliniL2Method(classifier=classifier,max_iter=args.cw_steps)
    if args.method == 'ZOO':
        attack = ZooAttack(classifier=classifier, max_iter=args.zoo_query, use_resize=True, use_importance=True)
    if args.method == 'SQUARE':
        attack = SquareAttack(estimator=classifier, eps=5, p_init = 0.1,nb_restarts =10,max_iter=args.square_query, norm = 2)
        #attack = SquareAttack(estimator=classifier, eps=0.03, max_iter=args.square_query, norm=np.inf)
    if args.method == 'BOUNDARY':
        attack = BoundaryAttack(estimator=classifier,min_epsilon = 0, targeted=False, max_iter=args.boundary_query)
    '''
    ...
    #np.save("./features/"+args.dataset+'/x_adv1', x_test)
    #x_test = np.clip(x_test,0,1)
    ...
    # predictions_adv = model(adv_samples)
    ...
    for i in range(len(adv_samples)):
        ...
        distortion.append(np.max(np.abs(adv_samples[i]-x_test[i])))
        #print("linf distortion",i, np.max(abs(adv_samples[i]-x_test[i])))
    ...
    '''
    if args.method =='PGD':
        print("PGD_eps:{}, and Robust Accuracy: {:.4f}".format(args.pgd_eps, accuracy))
    if args.method =='CW':
        print("Query:{}, and Robust Accuracy: {:.4f}".format(args.cw_steps, accuracy))
        #np.save('./features/cifar10/distortion_cw1',distortion)
    if args.method =='ZOO':
    ...
    if args.method =='SQUARE':
        ...
        #np.save('./features/cifar10/distortion_square1',distortion)
    if args.method =='HSJ':
        print("HSJmaxiter:{}, and Robust Accuracy: {:.4f}".format(args.hsj_iter, accuracy))
        #np.save('./features/cifar10/distortion_boundary1',distortion)


    np.save("./features/"+args.dataset+'/x_adv4', x_test)
    np.save("./features/"+args.dataset+'/'+args.method+'_adv4', adv_samples)
    
    np.save("./features/"+args.dataset+'/'+args.method+'_labels4', labels)
    np.save("./features/"+args.dataset+'/'+args.method+'_success4', success)
    np.save('./features/'+args.dataset+'/'+args.method+'_distortion4',distortion)
    np.save('./features/'+args.dataset+'/'+args.method+'_pred_test4',predictions_test)
    np.save('./features/'+args.dataset+'/'+args.method+'_pred_adv4',predictions_adv)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", '--dataset', type=str, choices=["mnist", "cifar10","tiny"], default="tiny")   
   # parser.add_argument("--method", type=str, default="CW", choices=['ZOO','BOUNDARY','CW','SQUARE'])
    parser.add_argument("--method", type=str, default="PGD", choices=['PGD','ZOO','HSJ','CW','SQUARE'])
    ...
    parser.add_argument("--hsj_iter",type = int, default = 50)
    ...
    #parser.add_argument("--zoo_query",type = int, default = 100)
    #parser.add_argument("--square_query",type = int, default = 2000)
    #parser.add_argument("--boundary_query",type = int, default = 10000)
