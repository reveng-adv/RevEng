...
def evalClassifier(test_loader,model,epoch = str(99)):
    model.load_state_dict(torch.load("./savedadvclfsq/clf_epoch"+epoch))
    model.eval()
    pred = []
    for batch_idx, (x, target, _) in enumerate(test_loader):
        x, target = x.to(device), target.to(device)
...
def main(args):
    ...
    y_pred = evalClassifier(test_loader,model,epoch = args.epochclf)
    ...
    print(classification_report(y_test, y_pred, target_names=("cwl2","sql2","bdrl2","pgd","sqlinf","hsj")))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--epochclf",type=str,default = "99")
    parser.add_argument("--init",default = None)# default="/vgg16")
    parser.add_argument("--output", default="./savedadvclfsq")
