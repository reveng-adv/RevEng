...
def all_true():
    ...
    zoo_p_adv = np.load(path+"SQUARE2_pred_adv10k.npy")
    ...
    zoo_l2 = np.load(path+"SQUARE2_distortion10k.npy")
    ...
    print(np.max(np.load(path+"SQUARE2_distortion10k.npy")[ind]))
...
def split():
    ...
    '''
    methods = ['CW', 'SQUARE2','BOUNDARY','PGD', 'SQUAREinf','HSJ']
    comb = []
    pertb =[]
    attk_method = []
    label = []
