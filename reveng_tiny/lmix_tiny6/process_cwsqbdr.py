...
def triple_true():
    ...
    sq_p_adv = np.load(path+"SQUARE_pred_adv10k.npy")
    ...
    sq_l2 = np.load(path + "SQUARE_distortion10k.npy")
    #print(np.shape(sq_l2))
    sq_s =(sq_l2<=5.001)& (np.argmax(sq_p_adv, axis=1) != labels) & (np.argmax(p_test,axis=1) == labels)
